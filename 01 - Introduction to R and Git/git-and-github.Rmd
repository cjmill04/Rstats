...
Since changes in your repository don't show up in mine, I won't know if you've created a really neat script that you want to share with the group. To do this, you'll need to open a terminal. Type <i>git remote -v </i> to check that you have two origins, one to fetch and one to push. Next, you want to set an upstream branch (my repository) to sync with your fork. To do this, type <i>git remote add upstream https://github.com/elizagrames/Rstats.</i> Verify that the change happened by doing <i>git remote -v </i> again. 
